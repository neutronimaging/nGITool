...
nGIISSfilter::nGIISSfilter() :
	PreprocModuleBase("nGIISSfilter"),
	m_fTau(0.01f),
	m_nN(10),
	m_fLambda(0.01f),
	m_fAlpha(0.0025f),
...
std::map<std::string, std::string> nGIISSfilter::GetParameters()
{
...
int nGIISSfilter::ScaleData(kipl::base::TImage<float,2> &img)
{
	if (m_bScaleData) {
...
int nGIISSfilter::RescaleData(kipl::base::TImage<float,2> &img)
{
	if (m_bScaleData) {
		float *pImg=img.GetDataPtr();
